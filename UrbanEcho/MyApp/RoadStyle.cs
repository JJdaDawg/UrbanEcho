...
//using Mapsui;
//using Mapsui.Experimental.Rendering;
...
//https://github.com/Mapsui/Mapsui/blob/01dcb06134a76b62d41e0b6d3a54151f9b57fc9e/Mapsui.Experimental.Rendering.Skia/Extensions/LineStringExtensions.cs#L17
    ...
    public class RoadStyleRenderer : ISkiaStyleRenderer
    {
        ...
        IFeature feature, LineString lineString, float opacity, RenderService renderService, int position)
        {
            ...
            SKPath ToPath((long featureId, int position, MRect extent, double rotation, float lineWidth) valueTuple)
            {
                ...
                List<SKPoint> vertices = WorldToScreen(viewport, coordinates);

                for (int i = 0; i < vertices.Count; i++)
                {
                    if (i == 0)
                    {
                        path.MoveTo(vertices[i]);
                    }
                    else
                    {
